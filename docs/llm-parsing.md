...
- LLM은 **비정형 화물 요청을 구조화하는 `POST /parse` 한 곳**에만 사용한다.
...
- 매칭 순위와 운임 적정성은 LLM에 맡기지 않는다. mock 기사·시세 데이터와 정적 규칙으로 계산해 재현 가능한 데모를 만든다.
...
- 운임 경고·매칭 결과는 LLM 응답과 분리돼 있으므로, 파싱 결과를 사용자가 수정한 뒤에도 동일한 규칙 로직으로 재계산한다.
